        ...
        if (!err) {
            if (results.length <= 0) {
                ...
                var mailOptions = {
                    ...
                    to: results[0].email,
                    subject: 'Password by Cunstruction site',
                    html: '<p><b> Your Login Details for Construction</b><br><b>Email:</b>' + results[0].email + '<br><b>Password:</b>' + results[0].password + '</p>'
                };
    ...
    connection.query(query,[email, user.oldPassword], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
                ...
                return res.status(400).json({ message: "Something went wrong. Please try angain letter" });
